...
from segmentation.config import (FOLD_NUM, INIT_TRAINER, SETUP_TRAINER,
                                 VERSION)
...
def main():
    ...
    path_list = (preprocessed_dir / "data_2d").glob("*.hdf5")
    path_list = [str(path) for path in path_list]
    ap_list = (preprocessed_dir / "data_3d").glob("*.hdf5")
